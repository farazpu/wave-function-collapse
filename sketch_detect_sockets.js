...
const SD_TH = 30; // socket detection threshold
...
function colorDistance(first, second) {
  ...
  let color1 = new Color(`rgb(${first[0]}, ${first[1]}, ${first[2]})`);
  let color2 = new Color(`rgb(${second[0]}, ${second[1]}, ${second[2]})`);

  return color1.deltaE(color2);
}


const claculatedSockets = []

function detectSockets() {
  tiles.forEach((t, i) => {
    let c = t.img.get(parseInt(t.img.width / 2), 0)
    let newSocketDetected = true
    claculatedSockets.forEach(cs => {
      let distance = colorDistance(cs[0], c)
      if(distance <= SD_TH) {
        cs.push(c)
        newSocketDetected = false
      }
    })
    if(newSocketDetected) {
      claculatedSockets.push([c])
    }

  });

  console.log(claculatedSockets)
}

function draw() {

  if(loadedTiles.length === 0) {
    // let thisTileIndex = tiles.indexOf(this)
    // if(!loadedTiles[thisTileIndex]) {
    //   loadedTiles[thisTileIndex] = true
    //   console.log(this.img.height, this.img.width)
    // }

    tiles.forEach((t, i) => {
      image(t.img, 0, 0, UNIT, UNIT);
      loadedTiles[i] = true
    });
    detectSockets()

    let h = CANVAS / claculatedSockets.length
    let y = 0
    claculatedSockets.forEach((cs, i) => {
      let w = CANVAS / cs.length
      let x = 0
      cs.forEach(c => {
        fill(color(c))
        rect(x, y, w, h)
